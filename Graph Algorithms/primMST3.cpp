...
/*void heapsort(vertex* arr[],int n){
    for(int i=n-1;i>0;i--){
...
int main(){
    ...
    for(int i=0;i<e;i++){
    	cin>>ss>>d>>w;
        ///vertices id starts from 0 to n-1
    	arr[ss]->adj.push_back(arr[d]);
    	arr[ss]->weight.push_back(w);
    	arr[d]->adj.push_back(arr[ss]);
    	arr[d]->weight.push_back(w);
    }
    arr[0]->key = 0; ///source
    ...
    int heapsize = v;
    int sum = 0;
    cout<<"vertices--->"<<"their key values"<<endl;
    while(heapsize>0){
        /*while(0){
